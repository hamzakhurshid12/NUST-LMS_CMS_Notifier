    ...
    if not os.path.exists(name):
        ...
        with open(name, 'wb+') as fd:
            for chunk in r.iter_content(chunk_size):
                fd.write(chunk)
        balloon_tip('LMS Notification','Downloaded '+nameShort)
    #addToDownloaded(url)
    
def addToDownloaded(text):
    file=open(os.path.join(folderPath,"downloaded.csv"),"a+")
    file.write(text+"\n")
    file.close()

##def isDownloaded(text):
##    file=open(os.path.join(folderPath,"downloaded.csv"),"r+")
##    data=file.read()
##    file.close()
##    if text in data:
##        return True
